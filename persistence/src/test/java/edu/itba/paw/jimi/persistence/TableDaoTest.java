...
public class TableDaoTest {
	...
	@Test
	public void testGetBusyTablesWithOrdersOrderedByOrderedAt() {
		Collection<Table> expectedBusyTables = busyTables.parallelStream()
				.filter(t -> t.getOrder().getStatus() == OrderStatus.OPEN)
				.sorted(Comparator.comparing(t -> t.getOrder().getUnDoneDishes().values().stream()
						.findFirst()
						.get()
						.getOrderedAt()))
				.collect(Collectors.toList());
		Collection<Table> actualBusyTables = tableDao.getBusyTablesWithOrdersOrderedByOrderedAt(busyTables.size(), offset);
		assertEquals(expectedBusyTables, actualBusyTables);
	}
