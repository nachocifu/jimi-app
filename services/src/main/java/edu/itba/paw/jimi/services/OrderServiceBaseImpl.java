...
public class OrderServiceBaseImpl implements OrderService {
	...
	public Collection<Order> findAll(QueryParams qp) {
		return findAllRelevant(qp.getPageSize(), qp.getStartAt());
	}

	@Override
	public Collection<Order> findAll(int pageSize, int offset) {
		Collection<Order> orders = orderDao.findAll(pageSize, offset);
		if (orders != null)
			return orders;
		else
			return new HashSet<Order>();
